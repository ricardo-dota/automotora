...
def inquiry(request):
    if request.method == 'POST':
        ...
        try:
            send_mail(
                    'New Car Inquiry',  
                    'You have a new inquiry for car.,' +car_title+'login for more info',
                    'devantoniomorales@gmail.com',
                    ['devantoniomorales@gmail.com'],# deberia ir admin_email
                    fail_silently=False,
                )
        ...
        messages.success(request, 'Se ha enviado su peticion correctamente, nos pondremos en contacto con usted' )
        return redirect('/cars/'+car_id)
    else:
        redirect('home')